<template>
  ...
  <table class= "table table-hover" style=" overflow : hidden; border-radius : 4px ; box-shadow: 0 4px 20px rgba(0, 0, 0, 0, 15);">
    ...
    <tbody>
    <tr v-for="(bestellung, index) in sortedArray" v-bind:key="bestellung.id">
      <td>{{ bestellung.id }}</td>
      ...
      <td><router-link :to="{ name: 'edit_product', params: { id: bestellung.id }}" class="btn btn-danger">Edit</router-link>
        <button class="btn btn-danger" v-on:click="deleteOrder(index)">Delete</button></td>
    </tr>
    </tbody>
  </table>
  </div>
